...
SoundManager.playSoundMXWithParams = function(fileName, volume) {
  var se = {};
  se.name = fileName;
  se.pitch = 100;
  se.volume = volume;
...
//==============================
// * Check Player Adjacent
//==============================
Game_CharacterBase.prototype.playerAdjacent = function(distance) {
  return Math.abs($gamePlayer.x - this.x) + Math.abs($gamePlayer.y - this.y) <= distance;
}
...
Game_Player.prototype.commandAttackUsable = function() {
  ...
  if (Input.isTriggered(Moghunter.ras_buttonWeapon)) {
    return true;
  }
  return false;
};
...
ToolEvent.prototype.playHitAnimation = function(target) {
  if (this.checkWeapon(this.user().battler())) {
    var animationID = this.user()
      .battler()
      .equips()[0].animationId;
  } else {
...
ToolEvent.prototype.deadEffectActor = function(char, battler) {
  ...
  if (!$gameSystem.isChronoMode()) {
    if ($gameParty.leader()._actorId === battler._actorId) {
      ...
      if ($gameParty.leader().isDead()) {
        this.prepareGameOver(char, battler);
      }
...
ChronoWindows.prototype.updateCommand = function() {
  ...
  if (this._command1.scale.x > 1.0) {
    this._command1.scale.x -= 0.1;
    if (this._command1.scale.x <= 1.0) {
      this._command1.scale.x = 1.0;
    }
  }
...
//==============================
// * Update Collapse JCD : Instantly Removes Character
//==============================
Sprite_Character.prototype.updateCollapse = function() {
  this._character.endAnimation();
  this.faceCharacterCR(-255, true, true);
};
