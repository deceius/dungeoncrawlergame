...
Yanfly.SpawnEventFailChecks = function(mapId, eventId, x, y) {
  if (!Yanfly.PreloadedMaps[mapId]) {
    if ($gameTemp.isPlaytest()) {
      console.log("Map ID " + mapId + " has not been preloaded. " + "It cannot be used for the Spawn Spawn function.");
    }
    return true;
  }
  if (!Yanfly.PreloadedMaps[mapId].events[eventId]) {
    if ($gameTemp.isPlaytest()) {
      console.log("Map ID " + mapId + ", Event ID " + eventId + " does not " + "exist. It cannot be used for the Spawn Event function.");
    }
    return true;
  }
  if ($gameMap.eventsXy(x, y).length > 0) {
    if ($gameTemp.isPlaytest()) {
      console.log("Cannot spawn event at " + x + ", " + y + "." + "There is already an event there.");
    }
    return true;
  }
  if ($gameMap.boat().posNt(x, y) || $gameMap.ship().posNt(x, y)) {
    if ($gameTemp.isPlaytest()) {
      console.log("Cannot spawn event at " + x + ", " + y + "." + "There is a vehicle there.");
    }
    return true;
  }
  if (x < 0 || x >= $gameMap.width() || y < 0 || y >= $gameMap.height()) {
    if ($gameTemp.isPlaytest()) {
      console.log("Cannot spawn event at " + x + ", " + y + "." + "This is outside of the map's boundaries.");
    }
    return true;
  }
  return false;
