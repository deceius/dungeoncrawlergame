...
Game_CharacterBase.prototype.isVictoryPose = function() {
  if (this.battlerPoses()) {
    return $gameSystem._chronoMode.phase === 5 && $gameTemp._chrono.victory[2] < 2;
  }
  return false;
};
...
Game_Character.prototype.setCharacterFrames = function() {
  ...
  if (ex) {
    this._frames.x = Number(ex[2]);
  }
  var ey = this._characterName.match(/(Y(\d+\.*\d*))/i);
  if (ey) {
    this._frames.y = Number(ey[2]);
  }
  var sp = this._characterName.match(/(S(\d+\.*\d*))/i);
  if (sp) {
    this._frames.speed = Number(sp[2]);
  }
...
Game_CharacterBase.prototype.needSetOriginalName = function() {
  ...
  if (this._characterName === this._originalName.name + "_dash") {
    return true;
  }
  if (this._characterName === this._originalName.name + "_jump") {
    return true;
  }
  if (this._characterName === this._originalName.name + "_idle") {
    return true;
  }
  return false;
};
