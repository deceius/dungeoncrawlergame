...
/*:
 * @plugindesc v1.0a - Fixed char.event().meta is undefined issue on dynamically spawned events (Deceius).
 * @author Caethyril
 *
 ...
 * Update log:
 *   1.0	: Initial release.
 * 	 1.0.a	: Fixed char.event().meta is undefined issue on dynamically spawned events.
 *
 * @param --- Defaults ---
 * @text --- Defaults ---
 * @type select
...
// ======================= Core functions ======================= //
	...
	// Checks whether events on a tile are blocking li	ne-of-sight, returns true or false.
		...
		if (!ignor) {									// If not ignoring events...
			let evs = $gameMap.eventsXy(tile.x, tile.y).filter(function(ev) {	// Get all events here
				...
				if (ev._pageIndex < 0) return false;
								// Event inactive: ignore
				if (!ev._evBlock) {				// Only if not tagged as block...
			...
			return !evs.every(function(ev) {			// Then don't block iff every remaining event...
				return !!ev._evNoBlock;	// ...has a <no sight block> tag
			});
		}
		return false;
	};
...
// ------------ YEP Event Chase Player plugin ------------ //
	...
	// Instead of checking the event's metadata, we will be intercepting the comments upon setting up 
	// the event page. This is a compatibility fix for dynamically spawned events. This was tested 
	// with Yanfly_SpawnEvent, and so far it works as intended.
	// Deceius, 7/24/2024
	var _cae_los_gevent_setupPageSettings = Game_Event.prototype.setupPageSettings;
    Game_Event.prototype.setupPageSettings = function() {
		// works like super() in other languages.
		_cae_los_gevent_setupPageSettings.call(this);
		// initialize LOS event default properties  
        this._evBlock = false;
        this._evPiercing = false;
        this._evNoBlock = false;
		// parse the comments and handle the properties mentioned above.
        this.setupLOSParams();
    };

	// Look up for Line of Sight tags in the comments, and return the parameter as true
	// should the comment be found in the event's page.
    Game_Event.prototype.setupLOSParams = function() {
        if (!this._erased && this.page()) {
          this.list().forEach(function(l) {
            if (l.code === 108) {
              var comment = l.parameters[0];
			  // with this implementation, one should define the tags one comment at a time.
              switch(comment.toLowerCase()) {
                case _.tags.evBlock:
                    this._evBlock = true;
                    break;
                case _.tags.evPiercing:
                    this._evPiercing = true;
                    break;
                case _.tags.evNoBlock:
                    this._evNoBlock = true;
                    break
                default:
                  // do nothing
              }
